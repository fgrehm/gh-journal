...
func buildReport(date time.Time) (map[string]interface{}, error) {
	...
	// TODO: tags / releases, open sourced projects (not created)
	...
	pipe := session.DB("gh-journal").C("events").Pipe([]bson.M{
		...
		},
		{
			"$project": bson.M{
				...
				"description":   "$raw.payload.forkee.description",
			},
		},
		{"$sort": bson.M{"user": 1}},
	})
...
func fetchIssuesUpdated(session *mgo.Session, startDate time.Time) (interface{}, error) {
	...
	pipe := session.DB("gh-journal").C("events").Pipe([]bson.M{
		...
		},
		{
			"$project": bson.M{
				"repository": bson.M{"$concat": []string{"$project.owner", "/", "$project.name"}},
				"author":     "$raw.payload.issue.user.login",
				"action":     true,
				"actor":      true,
	...
	pipe := session.DB("gh-journal").C("events").Pipe([]bson.M{
		...
		{"$sort": bson.M{"created_at": 1}},
		{
			"$project": bson.M{
				...
				"created_at": true,
				"actor":      true,
				"author":     bson.M{"$ifNull": []string{"$raw.payload.pull_request.user.login", "$raw.payload.issue.user.login"}},
